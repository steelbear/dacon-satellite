...
from utils import dice_score_torch
    ...
    for epoch in range(EPOCHS):  # 10 에폭 동안 학습합니다.
        ...
        for i in range(16):
            for images, masks in tqdm(dataloader[i]):
                ...
                dice = dice_score_torch(outputs, masks.unsqueeze(1))
                loss.backward()
                optimizer.step()

                epoch_loss += loss.item()
                epoch_dice += dice.item()
            print(f'Epoch {epoch * 16 + i + 1}, Loss: {epoch_loss / len(dataloader[i])}, Dice: {epoch_dice / len(dataloader[i])}')
